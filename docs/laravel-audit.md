...
| R3 | ~~MEDIUM~~ **RESOLVED — does not affect live** | The archived `.env` had `APP_URL=http://127.0.0.1:8000` alongside `APP_ENV=production`, which would have emitted broken absolute image URLs. The **live** `.env` (supplied later) has `APP_URL=https://thepurplepanther.in` and a matching `ASSET_URL`, so production is correct. The archived copy carries a stale local edit and is **not a reliable deployment reference** — take environment values from the server, not the archive. The Node app validates `MEDIA_BASE_URL` at boot regardless. |
