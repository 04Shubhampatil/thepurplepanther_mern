 ...
 * theme's stylesheet and its script.js both select on it: `.menubar[href="#menu"]` opens
 * mmenu, `.cart-search-btn` opens the search overlay, and `.su-header-4-middle` is what
 * the scroll handler pins. Renaming a class here silently unhooks a behaviour rather than
 * breaking a build.
 *
 * Two changes, both required by the SPA:
 *   - internal `<a href>` became `<Link to>`, so navigating does not reload the document
 *     (React Router renders the same `<a>` with the same attributes)
 *   - the offer ticker is a React slider (theme/sliders.js) rather than one of script.js's
 */
export default function SiteHeader() {
  const offers = useOffersStore((s) => s.offers)

  // The ticker cannot measure an empty wrapper, so it waits for the coupons to arrive.
  useSliders(['one-grid-slider'], offers.length > 0)
  ...
  return (
    <>
            ...
            <div className="container container-fluid">
              <div className="row justify-content-center">
                <div className="col-md-4 mx-auto">
                  <div className="swiper one-grid-slider overflow-hidden" data-pp-offer-slider>
                    <div className="swiper-wrapper">
                      {offers.length > 0
                        ? offers.map((offer) => (
                        <div className="swiper-slide" key={offer.id}>
                        ...
                        <div className="swiper-slide">
                          <div className="discount text-center">
                            <div className="mb0 text text-dark">SHOP NEW ARRIVALS</div>
                          </div>
                        </div>
                        )}
                    </div>
      ...
      <div id="page" className="stylehome4 mobile-menu-home21-style dark-menu">
        <div className="header su-header-4-middle home21-style">
          <div className="menu_and_widgets">
            <div className="mobile_menu_bar">
              <div className="menu-bar text-white d-flex align-items-center">
                <a className="menubar" href="#menu"><span></span><span></span></a>
                <span className="text-white menu-text">Menu</span>
              </div>
              <button className="search cart-search-btn bg-transparent border-0 d-flex align-items-center gap-2 text-white">
